 ...
 * вызова — ключа на момент написания не было. Модель и цены проверьте по
 * консоли xAI до первого боевого прогона: сверьте `costUsd` из `live-check`
 * со счётом провайдера за тот же вызов.
 */

export const DEFAULT_GROK_ENDPOINT = "https://api.x.ai/v1/responses";

/**
 * Модель по умолчанию — быстрая без рассуждений: нам нужен не ход мысли, а
 * факт, и рассуждения только раздувают расход.
 */
export const DEFAULT_GROK_MODEL = "grok-4-1-fast-non-reasoning";
 ...
 *
 * Цифры НЕ сверены с живым счётом — см. заметку в шапке файла.
 */
