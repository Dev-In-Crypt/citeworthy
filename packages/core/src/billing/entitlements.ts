 ...
 *
 * **Осторожно при изменении.** Прогон начинается целиком или не начинается
 * вовсе, а типовой аудит — это `DEFAULT_GENERATED_PROMPT_COUNT` (24)
 * вопроса × 3 сэмпла × 3 ассистента = 216 ответов. При значении ниже 216
 * аудит на наборе вопросов по умолчанию не пройдёт вовсе: продукт скажет,
 * сколько нужно и сколько осталось, но первый же заход упрётся в отказ.
 * Значит, вместе с числом надо решать и сколько вопросов задаётся в
 * бесплатном аудите.
 */
export const FREE_CHECK_ALLOWANCE = 200;
