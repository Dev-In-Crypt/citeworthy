  ...
  const rows = await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        // `extraClientAccounts` здесь намеренно нет: событие провайдера его
        // не несёт, и перезапись обнулила бы докупленные аккаунты — агентство
        // с сорока клиентами получило бы потолок в двадцать пять.
        updatedAt: new Date(),
      },
    })
    .returning();
