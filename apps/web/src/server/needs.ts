...
/**
 * Кто настроен в расписании, но больше не спрашивается.
 *
 * Так бывает после перехода на младший тариф и после решения перестать
 * измерять платформу: строку расписания никто не переписывает. Причина для
 * агентства одна и та же — настроено, но не измеряется, — поэтому и вывод
 * один.
 *
 * Пустой список разрешённых означает «права ещё не загружены»: тогда пусто,
 * а не тревога. Поднять её на незагруженных правах значит показать её всем
 * сразу.
 */
export function droppedAssistants(
  row: Pick<PortfolioRow, "scheduledAssistants">,
  allowedAssistants: readonly string[],
): string[] {
  if (allowedAssistants.length === 0) return [];
  return row.scheduledAssistants.filter((id) => !allowedAssistants.includes(id));
}
...
export function needsFor(row: PortfolioRow, allowedAssistants: readonly string[] = []): NeedsRow[] {
   ...
   * настроено.
   */
  const dropped = droppedAssistants(row, allowedAssistants);
