...
- Следующая задача: **T17** (run-оркестрация: fan-out prompt×platform×sample). T13–T15 (live-адаптеры, [H]) — их live-часть требует ключей; при их отсутствии переходить к T16 (worker-скелет).
- Сделано: T00–T07, T10–T12, T16 отмечены; T08 написан но не отмечен (нет remote). Тестов: 48 core + 8 db + 10 tenancy + 8 e2e. Тесты: 4 (db) + 10 (tenancy) + 11 (storage) юнит, 8 e2e.
