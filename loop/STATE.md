...
- Следующая задача: **T51** (публичная white-label страница отчёта /r/[token]). T13–T15 (live-адаптеры, [H]) — их live-часть требует ключей; при их отсутствии переходить к T16 (worker-скелет).
